...
/*   Updated: 2022/12/31 22:08:27 by megrisse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (angle > 0 && angle < 90)
		if (map->map[(int)((f->y - 1) / TILE_SIZE)][(int)((f->x + 1)
			/ TILE_SIZE)] == '1' || map->map[(int)((f->y + 1)
			/ TILE_SIZE)][(int)((f->x - 1) / TILE_SIZE)] == '1')
			return (EXIT_SUCCESS);
...
int	check_if_wall(t_map *map, double x, double y)
{
	...
	if (x >= 0 && x < map->width * TILE_SIZE && y >= 0
